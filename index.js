...
function mergeFrom(x1, y1, x2, y2){
  ...
  tempX=x2;
  tempY=y2;
  maxAdd=((currentLength+1)*speed)*(((currentLength+1) * speed)>maxAdd) + maxAdd*(maxAdd>(delay + (currentLength+1) * speed));
  addPath(tempX, tempY, delay + (currentLength+1) * speed);
  while (currentLength) {
    if (
      tempX &&
...
function sahilSearch() {
  queue1.push([startX, startY, 0]);
  queue2.push([destinationX, destinationY, 0]);
  //store queue number that visited the square
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < columns; j++) {
      visited[i][j]=0;
    }
